...
function next(){
	if(questionCounter === quiz.length){
		quizOver();
		btnLeave.classList.remove("btnLeave")
	}else{
		getNewQuestion();
	}
	
	if (resultBox.classList.contains("hide") ){
	}else{
		  btnToggle.classList.remove("btnside")
	}
}	

function quizOver(){
	btnToggle.style.remove="display:none;"
	//ocultar quiz Box
	quizBox.classList.add("hide");
	//mosrar caja de resultado
	resultBox.classList.remove("hide");
	quizResult();
	btnToggle.classList.toggle("iconSide")
}
//obtener el Resultado del quiz al finalizar 
function quizResult(){
...
function startQuiz(){
	...
	answersIndicator();
	resetMenu.classList.toggle("btnside")
	btnToggle.classList.toggle("iconSide")
	uploadView.style.display = "none"
	resetMenu.classList.remove("sideLvl")
	btnLeave.classList.add("btnLeave") 
	
	if(btnToggle.classList.contains("rotate")){
		rotate()
	}else{
...
function leavequiz(){
	...
	attempt = 0;
	btnLeave.classList.remove("btnLeave")
	btnToggle.classList.add("iconSide")
	 quizOver()
}
